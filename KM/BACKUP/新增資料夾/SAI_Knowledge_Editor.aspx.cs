    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (rblObjectType.SelectedIndex.Equals(0) && cbUploadFile.Checked)
        {
            ...
            if (System.IO.File.Exists(tempFilePathDir))
            {
                ...
                {
                    Response.Write("<Script language='JavaScript'>alert('檔案已經存在,覆蓋舊檔案');</Script>");
                }
        ...
        using (SqlConnection sqlcon = new SqlConnection(_con))
        {
            ...
            if (updateRowCheck.Equals(0)) // 新增資料
            {
				...
				"",
				"alert('"+Request["id"]+"');window.location='SAI_Knowledge_Manager.aspx';", true);
				return;
        ...
        ScriptManager.RegisterStartupScript(this, this.GetType(),
            "",
            "alert('DONE');window.location='SAI_Knowledge_Manager.aspx';", true);
