...
public partial class uploadFile : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (FileUpload1.HasFile)
        {
            ...
            {
                Page.RegisterStartupScript("", "<script language='javascript'>window.alert('上傳內容必須是檔案');</script>");
                return;
            }
            if (!FileNameCheck(FileUpload1.FileName))
            {
                Page.RegisterStartupScript("", "<script language='javascript'>window.alert('上傳內容必須符合規範檔案');</script>");
                return;
            }
            if (!FileUploadCheck(FileUpload1.FileName))
            {
                // TODO 可以改成複寫
                Page.RegisterStartupScript("", "<script language='javascript'>window.alert('提醒：上傳檔案與分類中有相同名稱, 請確認後送出將會覆蓋檔案！');</script>");
                //return;
            }
